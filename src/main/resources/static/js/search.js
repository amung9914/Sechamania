...
let lastPathSegment = decodeURIComponent(pathSegments[pathSegments.length - 1 ]); // 마지막 주소에 담긴 값 == 검색어


document.addEventListener('DOMContentLoaded', function () {
    console.log("검색어 : "+lastPathSegment)
    ...
    function fail(){
        console.log("검색결과를 정상적으로 불러올 수 없습니다");
    }
    let body = JSON.stringify({
        "keyword":lastPathSegment
    })
    httpRequestWithResponse("POST","/api/search",body,success,fail);
})
...
function renderList(response){
    document.getElementById("search_title").innerHTML = '<h3>"'+lastPathSegment+'" 검색결과 : '+response.totalElements+'건</h3>';
